...
function filterTransactionId() {
  let transactionInpValue =
    document.getElementsByName("transactionID")[0].value;
...
function filterDate() {
  let dateParse = document
    .getElementsByName("date-input")[0]
    .value.match(new RegExp("([0-9]+)-([0-9]+)-([0-9]+)"));
...
function filterInvDate() {
  let dateParse = document
    .getElementsByName("invoiceDate")[0]
    .value.match(new RegExp("([0-9]+)-([0-9]+)-([0-9]+)"));
...
function formatTransAndAddresses() {
  ...
  function formatOutputText(element) {
    ...
    const startSubStr = value.substring(0, 6);
    const endSubStr = value.substring(strLength - 4);
...
function build_filter_args() {
  ...
  [
    ...
    { element_name: "memberID", arg_name: "member_id" },
...
function update_tx_table(page = 1) {
  ...
  document.querySelector("#report-download").href =
    `/parts/transactions?${report_download_args}`;
...
[
  ...
  document
    ...
    .addEventListener("keyup", (e) => update_tx_table());
});
...
[
  ...
  document
    .getElementsByName(e)[0]
    .addEventListener("change", (e) => update_tx_table());
});
