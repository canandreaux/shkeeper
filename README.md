...
The same API key is used as the **HMAC secret** when you [verify webhook callbacks](#webhook-signature-verification) from SHKeeper (`X-Shkeeper-Signature` / `X-Shkeeper-Timestamp`). Incoming REST API calls and outbound webhooks use the key in different ways: API requests authenticate with the header value; webhooks must be verified against the raw body signature.
...
The payout callback is signed the same way as invoice payment callbacks (`X-Shkeeper-Timestamp`, `X-Shkeeper-Signature`); see [Webhook signature verification](#webhook-signature-verification).
...
If enable_payout_callback is enabled in SHKeeper, a callback will be sent automatically upon a successful payout.
If a callback_url is provided in the request, SHKeeper will send the notification to that URL.
Payout callbacks include HMAC signature headers; see [Webhook signature verification](#webhook-signature-verification).
...
Once SHKeeper receives a payment, it will send a payment notification to the `<callback_url>` using a POST request with a JSON-encoded body.

**Authentication headers on the callback request:**

| Header | Description |
|---|---|
| `X-Shkeeper-Timestamp` | Unix timestamp (seconds) used when signing the request |
| `X-Shkeeper-Signature` | HMAC-SHA256 hex digest of `{timestamp}.` + raw request body |
| `X-Shkeeper-Api-Key` | Your wallet API key (legacy; kept for backward compatibility) |

For new integrations, **verify `X-Shkeeper-Signature`** using the same API key as the secret. Do not rely on `X-Shkeeper-Api-Key` alone — it can be spoofed by anyone who knows or guesses the key. Full algorithm, examples (Python, Flask, PHP), and security notes: **[Webhook signature verification (HMAC-SHA256)](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md)**.

<a name="webhook-signature-verification"></a>
#### 5.3.1 Webhook signature verification (HMAC-SHA256)

SHKeeper signs outbound webhooks (invoice payment notifications and payout success callbacks) with HMAC-SHA256.

**Signing message:**

```text
HMAC_SHA256(api_key, "{timestamp}." + raw_request_body)
```

- `timestamp` — value from `X-Shkeeper-Timestamp` (ASCII decimal string).
- `raw_request_body` — exact HTTP body bytes as received; do not parse and re-serialize JSON before verification.
- `X-Shkeeper-Signature` — lowercase hex digest (64 characters).

**Recommended handler flow:**

1. Read raw body bytes from the HTTP request.
2. Read `X-Shkeeper-Timestamp` and `X-Shkeeper-Signature` from headers.
3. Recompute HMAC with your wallet API key and compare signatures (constant-time).
4. Reject requests outside the allowed time window (default: 5 minutes) to limit replay attacks.
5. Process the JSON payload only after verification succeeds.

**Documentation:** complete guide with copy-paste examples — [webhook_verification.md](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md).
...
Official modules below handle SHKeeper API integration. If you build a **custom** payment or payout module, implement [webhook signature verification](#webhook-signature-verification) on your callback endpoint — see the [full guide](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md).
